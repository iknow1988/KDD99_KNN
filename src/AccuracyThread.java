...
public class AccuracyThread extends Thread {
	...
	private int[][] accMatrix = new int[2][5]; // for K=3
	...
	public void run() {
		...
		for (int i = from; i < to; i++) {
			...
			if (actual != null) {
				...
				accMatrix[0][idMap.get(predicted)] = accMatrix[0][idMap
						.get(predicted)] + 1;
				HashMap<String, Integer> count = new HashMap<String, Integer>();
				for (DataPoint point : tops) {
					if (count.containsKey(getAttackType(point.getLabel()))) {
				...
				accMatrix[1][idMap.get(predicted)] = accMatrix[1][idMap
						.get(predicted)] + 1;
				// System.out.print(predicted);
				// System.out.println();
			}
